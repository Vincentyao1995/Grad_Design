...
def K_cluster_dir(path = ''):
    ...
    cnt=0
    for name in track:
          df_all=pd.read_csv(ROOT+name,sep='\t',header=None,index_col=False,names=['time','utc','ec','lng','lat'])
          ...
          for day in days: 
              ...
              if len(result)>1:
                 if not os.path.isdir(ROOT+ 'stay_point'):
                     os.mkdir(ROOT + 'stay_point')
                 result.to_csv(ROOT+'stay_point/'+name[:-4]+'_'+day+'.txt',index=False)
          cnt+=1
          print(cnt)
